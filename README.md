...
对包含多组件、轴向层和局部插入件的真实模型，先使用 Facts-only
`controlled` gate 阻止错误 Facts 进入下游；不要把 `advisory` 当作修复模式：
...
conda run --no-capture-output -n openmc-env python -u -m openmc_agent.inspect \
  ...
  --plan-gates facts --patch-output-mode json_object --patch-max-tokens 12000 \
  --patch-reasoning-effort none --max-plan-additional-llm-calls 20
```

这只启用 Facts Gate；其它 Plan Gate 仍未在 controlled 模式中启用。
