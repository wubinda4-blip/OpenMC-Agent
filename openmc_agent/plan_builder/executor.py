    ...
    def _run_axial_geometry_gate(
        *, finalize_non_applicable: bool = False,
        ...
        if policy.mode is PlanLoopMode.CONTROLLED:
            ...
            # Material-Universe and Placement gates only become *ready*
            # (i.e. have all their required patches available for review)
            # once those patches have been generated by the executor loop.
            # When the upstream gate is still PENDING because its patches
            # have not been generated yet, we DEFER (return None) so the
            # executor loop can continue generating those patches instead
            # of prematurely blocking axial-geometry.  Once the patches
            # exist and the upstream gate is still not accepted, it is a
            # real blocker and we return the error.
