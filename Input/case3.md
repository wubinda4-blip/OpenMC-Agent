...
- 导向管 guide_tube：未插棒构型下导向管内为水，经典 C5G7 仅有薄壁 Zircaloy-4 管。沿用本算例"单材料均质棒"简化（与燃料棒不建包壳一致），导向管栅元整体填水：圆柱半径 0.54，材料同 water，对应 universe 为 guide_tube_pin。
- 裂变室：中心测量管未插棒时内为水，处理同导向管，整体填水：圆柱半径 0.54，材料同 water，对应 universe 为 fiss_chamber_pin。
- 水：H-1 66.67、O-16 33.33 ao%，密度 0.997，热散射 c_H_in_H2O。
- Zircaloy-4：Zr 98.23、Sn 1.45、Fe 0.21、Cr 0.10 wo%；密度 6.56。本算例当前不使用（均质棒简化下燃料棒无包壳、导向管/裂变室填水），保留定义供未来引入包壳/管壁结构时启用。
