...
- Incremental 已是复杂 3D 默认路径；VERA3 3B 默认 reference structural patches，monolithic reflect fallback 默认禁用。真实 LLM facts/materials/universes 稳定性仍需持续评估（真实 LLM 测试 opt-in，CI 不跑）。
