...
- **VERA3 component-profile gap**：canonical contract 已确认燃料端塞/气腔是 fuel-pin internal geometry，Pyrex/套管塞是有限轴向 guide-tube inserts；当前 patch fixture 仍含 whole-layer material slab、Pyrex gap/radial-stack 与 thimble loading 错误。test-only diagnostic oracle 会稳定报告这些问题；production schema/renderer 尚未修复。
