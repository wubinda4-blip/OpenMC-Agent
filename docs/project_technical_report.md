...
- **SenseNova `ds:` 推理模型输出截断修复**：`ds:deepseek-v4-flash` 默认开思考模式，CoT 经 `reasoning_content` 吃光输出 token 预算导致 JSON 写一半即断（facts 阶段 `json_parse_error`/`full_plan_output_forbidden`）。三处改动：(1) `DSChatClient.adjust_payload` 注入 `reasoning_effort`（默认 `low`，`SENSENOVA_REASONING_EFFORT` 可覆盖）；(2) 把此前死配置 `PATCH_MAX_TOKENS` 真正接进 `executor→generate_patch`，并按多组件/全堆调大预算（facts 1200→3000 等）；(3) 新增 `_looks_truncated` 括号平衡启发，截断时报独立码 `patch_generation.json_truncated`（带"调大预算/降 reasoning_effort"提示）而非笼统 `json_parse_error`。非-openmc `1930 passed`，benchmark `21/21`。
