...
- **Phase 8C Step 3 MU Canonical Binding — 确定性漂移修复 + Preflight Blocking**：分析 v9 MU Gate 16 个 preflight issues 的 4 个根因，实施 5 项修复。(1) UniverseSpecPatch 新增 `metadata` 字段 + 碎片化 pipeline 在 merge 后注入 `geometry_profile_id`/`source_requirement_ids`/`localized_insert_requirement_id`/`fuel_variant_id`；(2) `validate_materials_against_requirement_set` 修复 dict-vs-model 访问 bug（`getattr(dict,'role')` 返回 None 导致 6 个假阳性 material_role_uncovered）；(3) MU preflight `localized_insert_universe_missing` 增加 profile-based fallback（检查 `metadata.localized_insert_requirement_id`）；(4) `extract_universe_requirements_from_inventory` 交叉引用 `localized_insert_profiles` 设置 `localized_insert_requirement_id`；(5) MU gate controlled 模式下确定性 preflight error 直接 BLOCK（不调用 reviewer）。Canary v9→v11 验证：Materials 6/6、Universes 5/5 稳定通过；metadata 注入成功（profile_id 出现在 universe 上）；localized insert matching 改善。残余 22 个 `material_reference_missing` 错误 — LLM 在 universe cell 中引用 MaterialsPatch 不存在的 material（`helium`/`water`），根因是 inventory profile 未声明 background/gap/coolant 层的 material role。下一步：扩展 inventory profile 合同。3575 passed / 2 skipped；benchmark 21/21。
