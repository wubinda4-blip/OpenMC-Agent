...
- **P0-D5A Expert Feedback Semantics & Skeleton Blocker Routing**：新增 `capability_blockers.py`（按 route_hint/code-prefix/error_catalog 确定性分类 structural/environment/human_fact/fidelity，VERA3B 真实 blocker=`lattice_transform.replacement_universe_missing`/`renderer.axial_loading_materialization_failed`）+ `expert_feedback.py`（ExpertQuestionGroup/ExpertFeedbackDecision/ExpertAssumptionAcknowledgement，23 条材料假设归并为 ~8 组，空回车=defer/accept_review_only 而非模糊 continue，accept 不改 composition_status/density/boron）。supervisor 确定性优先级改为 structural_blocker_precedes_human_confirmation；assess 节点加 axial materialization dry-run probe 使结构 blocker 在专家交互前可见（P0-D5 最小接线，未实现 shoulder-gap repair）；skeleton 终态显示 BLOCKED_REVIEW_ONLY（内部 ok 仍 False），写 5 个 artifacts + 8 个 trace events。真实 VERA3B fixture 冻结：`data/runs/VERA_3B/expert_feedback_failure_diagnosis.json` + `tests/fixtures/regressions/vera3b_empty_expert_feedback_skeleton.json`；6 个新测试文件 `40 passed`，非-openmc 955 passed。
